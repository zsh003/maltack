...
var _default = api => {
  ...
  api.onStart(() => {
    var _api$config, _api$pkg$dependencies;

    if ((_api$config = api.config) === null || _api$config === void 0 ? void 0 : _api$config.mako) return; // don't print ad in bigfish framework

    if (process.env.BIGFISH_VERSION) return;
    if (process.env.MAKO_AD === 'none') return;
    if ((_api$pkg$dependencies = api.pkg.dependencies) === null || _api$pkg$dependencies === void 0 ? void 0 : _api$pkg$dependencies['umi-plugin-mako']) return;
    console.info(chalk.yellow.bold('Mako https://makojs.dev is a new fast Rust based bundler from us, which is heavily optimized for umi and much faster than webpack. Visit https://makojs.dev/docs/getting-started#bundle-with-umi for more details if you want to give it a try.'));
  });
